...
## 如何定义一个转写请求
转写以roomId： 会议室id + taskId： 任务id 为一个转写任务
...
## 需求
根据以上信息， 我们需要实现一个音视频转写成文字的功能， 

### 技术栈
转写模型使用阿里云的asr, paraformer-v2 
...
录音文件存储在 腾讯云的cos上.
说话人数量不固定,
转写后处理, 在前端能看到转写的结果.整体转写结果,及每个人的转写结果(时间,说话人, 内容)
转写后的数据存储在PostgreSQL数据库中
COS 访问方式 - 公共读取
说话人需要关联会议系统中的真实人名, 可以用会议系统的数据库中取
cos的路径在可以 .env文件中找到配置

### 技术要求
1. 在前端，开发一个转写列表的页面，放在后台的页面上。每个会议，有一个转写，可以下载文字转写结果，并且可以下载原始视频或者音频。 


### 技术方案
会议系统，结束会议，需要触发转写任务（记录在redis中）
后台有一个worker，会定期检查数据库中的转写任务，如果有任务，就去转写，转写完成后，将结果存储到数据库中，并更新数据库中的任务状态。


重点！！！： 转写结果文件中，必须保证真实的时序，说话人需要关联会议系统中的真实人名
重点！！！： 转写结果文件中，必须保证真实的时序，说话人需要关联会议系统中的真实人名
重点！！！： 转写结果文件中，必须保证真实的时序，说话人需要关联会议系统中的真实人名


## 方案
Worker 预合并	单流转写 + 合并	直接返回	查询快	Worker 复杂度增加

## 边界
组件	职责	不做什么
Worker	转写、合并、存储	不响应 HTTP 请求
Server	API 接口、权限控制	不做转写处理
Frontend	展示、交互	不做任何计算